...
reflectProperty.blacklist = {
  ...
  function reflectEvent(element, name, meta) {
    try {
      if (name[0] === '_' || name[name.length-1] === '_') {
        return;
      }
...
function reflectProperties(element) {
  ...
  function reflectEvents(element) {
    var props = [];
    if (element) {
      var found = {};
      var meta = element._designMeta && element._designMeta.properties;
      ...
      while (p && !p.hasOwnProperty('PolymerBase') && p != hep && p != hiep) {
        ...
        k.forEach(function(k) {
          ...
          var prop = reflectEvent(element, k, meta);
          if (prop) {
            props.push(prop);
            found[k] = true;
          }
...
function reflectStyles(element, meta) {
  ...
  function reflectObjectEvents(obj) {
    var events = [];
    if (obj) {
      Object.keys(obj).forEach(function(name) {
        var e = reflectEvent(obj, name, {});
        if (e) {
          events.push(e);
          console.log("Found an event: e="+ e.toString());
        }
      });
    }
  }
...
window.Reflection = {
  ...
  objectEvents: reflectObjectEvents
};
